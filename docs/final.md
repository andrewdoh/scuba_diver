...
$$ \alpha = 0.1, \gamma =0.9, \epsilon = 0.1$$
 ...
 <div align="center">
<img src="images/singlevsdouble_ex_2_3.png" width="350px">
<img src="images/singlevsdouble_ex_2_5.png" width="350px">
<img src="images/singlevsdouble_ex_2_10.png" width="350px">
 </div> 
  
 We can see from first glance that our agent is steadily increasing and accumulating reward as the number of episodes increase. Each episodes, our agent begins by taking actions that result in some form of reward which can be negative or positive. The agent's cumulative reward per episode will indicate whether it's performing its given task which is to search for the red stone block while simultaneously try to find as much treasure as it can. This is computed directly through the number of actions the agent takes which over time should begin to converge to an efficient sequence of actions resulting in maximum reward. While each sequence of action directly affects the maximum reward the differences in double Q-learning and single Q-learning can easily be seen from the cumulative rewards as well. In single Q-learning, by always taking the $$\max(S,A)$$ there is a bias that is occurs from always taking this The maximization bias single Q-learning has is decoupled in this instance by taking the average of two tables counteracting this effect. With the limited time we had we opted to run each map $$50$$ episodes which at first evidently seems to be well behaved but as the difficultly of the map increases tends to bounce between lows and highs. While there are times where cumulative reward drops drastically, we attribute this fluctuation to the relative sizes of the map as the agent trying explore more area which increases. In it worthwhile to note that this doesn't affect the performance or efficiency of our agent in the long run. Additionally, we also can see how double Q-learning affects our action space as the graphs are much more smooth in comparison to single Q-learning, which is much more irregular. 
 
The second metric to evaluate the efficiency of our agent is time spent per episode. This is another very intuitive idea that is simple to state but yields an incredible amount information in a easily convincible manner. For our the purposes for our agent, we can see time as a way to describe how well its performing in regards to how efficiently time is spent finding the red stone block and treasure while trying to maintain a good supply of air as not to die. if the agent wasn't efficient with its time then it would have died or been wasted exploring too much and not completing its goal. Again, looking at the graphs we can see that both algorithms perform its intended purpose but single Q-learning algorithm is much more scattered. Nonetheless, a decrease in the time spent per episode is an indication of time well spent which can be seen in the graphs for increase episodes. 

As one might expect, in the initial episodes, the life of the agent should be relatively short as it's just beginning to explore and learn more information about the world. As time progresses, however, we expect that it should be gradually learning about it's world state and how to best efficiently complete its goal. At this point, it should be near convergence as the relative time spent per episode should be getting smaller. It is clear, that as the time spent per episode decreases the agent has found a more efficient sequence of actions to complete its goal and maximize its cumulative reward. Denoted below is the proof: 
<p align="middle">
<h5 align="center"> <b>Example 1 </b></h5>
$$ \alpha = 0.1, \gamma =1, \epsilon = 0.1$$
</p>
...
$$ \alpha = 0.1, \gamma =0.9, \epsilon = 0.1$$
 ...
 <div align="center">
 <img src="images/singlevsdouble_ex_2_3_time.png" width="350px">
 <img src="images/singlevsdouble_ex_2_5_time.png" width="350px">
  <img src="images/singlevsdouble_ex_2_10_time.png" width="350px">
 </div>
The results were more or less what we expected. It begins starting off with short life spans, which begins increasing over time. With a map of our size and state space as large as we have our agent had troubles converging, and so we weren’t able to observe if the agent would start decreasing the amount of time spent per mission near convergence. To remedy this problem, we sought to implement a function approximator. The idea behind our function approximation was to use Euclidean distance and relative direction of the treasure I.e.(north, east, south, west). We initially tried to use these two methods to reduce our state space but with the limited amount of time we had it did not function very efficiently and actually increased the state space. While our agent times some time to converge it nevertheless does. Overall, if the agent is able to both increase the amount of time it stays alive by sustaining the appropriate amount of air as well increasing the amount of rewards it receives per mission, we are satisfied in knowing that our agent is moving towards convergence and finding an optimal policy. And with the information that we have been able to gather thus far, we believe it is doing just that.  
  
From our analysis, we tested several different levels of $$\alpha$$, $$\gamma$$, and $$\epsilon$$. For lower levels of $$\gamma$$, the agent would discount future rewards as not very important leading to decreased cumulative rewards. while setting alpha levels high tended to make the agent opt to not explore for treasure as often.  In regards to the epsilon value, we are satisfied with keeping it around a value of 0.1, as that appears to be a common tactic in reinforcement learning in order to prevent the agent from acting ‘too’ randomly. Regardless, we plan on monitoring all of these values more closely in the future. A more efficient manner to finding the optimal parameters would have been to optimize for them but without sufficient time we opted to test varying degrees of value. So, after extensive testing the parameters we concluded that yielded the best reward sand found the optimal are $$ \alpha = 0.1, 0.9 \leq \gamma \leq 1, \epsilon = 0.1$$.  
